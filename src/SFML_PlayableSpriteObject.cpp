...
SFML_BulletSpriteObject* SFML_PlayableSpriteObject::processEvents(float elapsedTime, sf::Transform transform, sf::Vector2f mouseScreenPosition)
{
	...
	if (leftButtonPressed || rightButtonPressed)
	{
		...
		sf::Vector2f charScreenPos = getScreenPos(transform);
...
void SFML_PlayableSpriteObject::reduceHealth(int dmg)
{
	m_health -= dmg;
}

void SFML_PlayableSpriteObject::resetHealth()
{
	m_health = 100;
}
...
void SFML_PlayableSpriteObject::setWeaponDmg(int dmg)
{
	m_weaponDmg = dmg;
}

int SFML_PlayableSpriteObject::getWeaponDmg()
{
	return m_weaponDmg;
}
